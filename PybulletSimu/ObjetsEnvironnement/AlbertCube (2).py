...
class AlbertCube(Cube):
    ...
    def __init__(self, room_manager):
        ...
        self.memory_observation = []  # max length = 5


        self.count_jumping_steps = 0
    ...
    def create_cube(self, pos, orient_euler, mass):
        ...
        return super().create_cube(new_pos, orient_euler, mass)

    def reset_position_orientation(self, pos, ori_euler):
        orient_quaternion = p.getQuaternionFromEuler(ori_euler)
        room_coord = self.room_manager.room_array[self.actual_room].global_coord
        new_pos = [pos[0] + room_coord[0], pos[1] + room_coord[1], pos[2] + room_coord[2]]
        p.resetBasePositionAndOrientation(self.id, new_pos, orient_quaternion)
    ...
    def raycasting(self):
        ...
        # use the following script to visualize the ray mapping
        """
        for n in range(21):
    ...
    def jump(self, jump, move):
        ...
        if self.start_jumping:
            ...
            self.jump_orientation = p.getEulerFromQuaternion(p.getBasePositionAndOrientation(self.id)[1])[2]
            impulse = [500 * self.x_jumping_factor, 0, jump_force]
            p.applyExternalForce(self.id, -1, impulse, [0, 0, 0], p.LINK_FRAME)
            self.count_jumping_steps += 1
            jumping = False
        elif (not self.start_jumping and self.still_jumping):
            if (self.count_jumping_steps >= 1 and self.count_jumping_steps <= 100):
                impulse = [500 * self.x_jumping_factor * np.cos(new_ori - self.jump_orientation),
                           -500 * self.x_jumping_factor * np.sin(new_ori - self.jump_orientation), jump_force]
                p.applyExternalForce(self.id, -1, impulse, [0, 0, 0], p.LINK_FRAME)
                self.count_jumping_steps += 1
    ...
    def get_observation(self):
        ...
        for i in range(len(contact_results)):
            ...
            if contact_results[i][0][0] == 0 or contact_results[i][0][0] == -1:
                current_observation[21 + i] = 10  # 10 = max distance of the ray, change this line if the ray length is changed in raycasting()
    ...
    def add_to_memory_observation(self, current_observation):
